    ...
    class Mom: Character
    {
        public Mom()
        {
            ...
            for (int i = 0; i < (361 * 7); i += w)
            {
                ...
                Character.setAngry(angrysprites);
                
            }
            for (int i = 0; i < (361 * 9); i += w)
            {
                happysprites.Add(new Sprite(t, new IntRect(i, 465, w, 465))); //second row of sprites; happy epression 
                Character.setHappy(happysprites);
            }
            for (int i = 0; i < (361 * 4); i += w)
            {
                neutralsprites.Add(new Sprite(t, new IntRect(i, 465*2, w, 465))); //second row of sprites; happy epression 
                Character.setNeutral(neutralsprites);
            }
